...
function init() {
  ...
  scene.background = new THREE.Color(); // sky blue
  ...
  ParticleManager.init(20000); // 2000 is the particle count
...
const ParticleManager = {
    ...
    init: function(count = 2000) {
        const geo = new THREE.BufferGeometry();
        const positions = new Float32Array(count * 3);
        this.velocities = new Float32Array(count * 3);

        for (let i = 0; i < count * 3; i += 3) {
            positions[i + 1] = -1000; // Start hidden underground
            this.velocities[i] = (Math.random() - 0.5) * 2;
            this.velocities[i + 1] = Math.random() * 5 + 5; 
        ...
        this.system.position.set(0, 0, 0); // System stays at origin for World Space
    },

    update: function(dt, playerPos) {
        if (!this.system) return;

        // Find biome where player is standing to handle global fade
        const playerBiome = (typeof getBiomeAt === 'function') ? getBiomeAt(playerPos.x, playerPos.z) : null;
        const weatherEnabled = playerBiome && playerBiome.particles && playerBiome.particles.enabled;

        // 1. FADE LOGIC: Particles fade in when you enter a weather biome
        const targetOpacity = weatherEnabled ? (playerBiome.particles.opacity || 0.8) : 0;
        this.opacity += (targetOpacity - this.opacity) * dt * 2;
        this.system.material.opacity = this.opacity;
        ...
        const posAttr = this.system.geometry.attributes.position;
        const positions = posAttr.array;
        const range = (playerBiome && playerBiome.particles && playerBiome.particles.range) || 60;
        ...
        for (let i = 0; i < posAttr.count; i++) {
            ...
            // 3. KILL & RESPAWN LOGIC
            let shouldReset = false;

            // Check biome at the PARTICLE'S position
            const pBiome = (typeof getBiomeAt === 'function') ? getBiomeAt(positions[idx], positions[idx + 2]) : null;

            // Kill if hit a block
            if (typeof getBlock === 'function' && getBlock(positions[idx], positions[idx + 1], positions[idx + 2])) {
                shouldReset = true;
            }
            // Kill if falls too low
            else if (positions[idx + 1] < playerPos.y - 10) {
                shouldReset = true;
            }
            // Kill if outside the follow box
            else if (Math.abs(positions[idx] - playerPos.x) > range || Math.abs(positions[idx + 2] - playerPos.z) > range) {
                shouldReset = true;
            }
            // Kill if particle drifts into a biome that doesn't have weather
            else if (!pBiome || !pBiome.particles || !pBiome.particles.enabled) {
                shouldReset = true;
            }

            if (shouldReset) {
                positions[idx]     = playerPos.x + (Math.random() - 0.5) * range * 2;
                positions[idx + 1] = playerPos.y + 30 + Math.random() * 20;
                positions[idx + 2] = playerPos.z + (Math.random() - 0.5) * range * 2;
            }
        }
...
function generateTree(x, y, z, cx, cz, rng, treeConfig) {
  ...
  if (shape === "pine") {
    ...
    for (let i = 0; i < leavesHeight; i++) {
      ...
      // Top layer has radius 0, bottom layer has max radius
      const layerProgress = i / leavesHeight;
      const currentRadius = radius * (1.0 - layerProgress);
      const layerY = y + leavesStartRelative + i;

      for (let lx = -Math.ceil(currentRadius); lx <= Math.ceil(currentRadius); lx++) {
    ...
    // 1. The Core (Top of trunk)
    placeBlockInChunk(leavesID, x, topY, z, cx, cz);
    ...
    if (rng() > 0.5) placeBlockInChunk(woodID, x + 1, topY - 1, z, cx, cz); // East Coconut
